...
def render_audit_journal_tab(conn, events_df, current_gw, player_pool_df=None):
    ...
    st.markdown("## 📊 Model Audit & Performance Journal")
    st.caption("Inspect locked solver versions, track pre-match line shifts, and audit prediction variance against final outcomes.")

    mgr_to_use = st.session_state.get("manager_id", "").strip()
    if not mgr_to_use:
        st.info("👆 Enter your FPL Team ID in the top header to load and audit your squad.")
        return
    ...
    with c3:
        if st.button("🔒 Lock New Version", key=f"lock_btn_gw_{selected_gw}", use_container_width=True):
            with st.spinner(f"Computing new version for GW{selected_gw}..."):
                ...
                if err:
                    ...
                    st.toast(f"GW{selected_gw} locked as Version {new_ver}!", icon="✅")
                    st.rerun()

    with c4:
        if st.button("🔄 Settle Outcomes", key=f"settle_btn_gw_{selected_gw}", use_container_width=True):
            if not snapshot:
                ...
                with st.spinner(f"Fetching official points for GW{selected_gw}..."):
                    ...
                    if not raw_live_map:
                        ...
                        st.toast(f"GW{selected_gw} Version {selected_version} settled!", icon="✅")
                        st.rerun()

    if not snapshot:
        st.info(f"No snapshot logged for **GW {selected_gw}**. Click **🔒 Lock New Version** or lock your team in the **Squad Analyzer** tab.")
        return
    ...
    m1.metric("Status", "Settled ✅" if is_settled else "Pre-Lock 🔒")
    m2.metric("Viewing Version", f"v{snapshot.get('version', 1)} of {len(all_versions)}")
    m3.metric("Projected Total", f"{pred_total_val:.1f} xP")
    m4.metric("Actual Score", f"{snapshot['actual_total']} pts" if is_settled else "Pending ⏳")
    ...
    if len(all_versions) > 1:
        st.markdown("#### 📜 Version Iteration History")
        hist_rows = []
    ...
    with c_view_title:
        st.markdown(f"### 🛡️ Locked Lineup: GW{selected_gw} (Version {selected_version})")
    with c_view_toggle:
        view_mode = st.radio("View Mode:", ["🏟️ Pitch", "📋 Table"], horizontal=True, key=f"view_mode_gw_{selected_gw}")
    ...
    if "Pitch" in view_mode:
        ...
        table_starters = pd.DataFrame([{
            "Role": "Captain 👑" if p.get("is_cap") else ("Vice 🎖️" if p.get("is_vc") else "Starter"),
            "Player": p["web_name"],
            "Team": p.get("Team", "-"),
            "Pos": p.get("Pos", "-"),
        ...
        st.dataframe(table_starters, use_container_width=True, hide_index=True)
        ...
        if not bench_df.empty:
            ...
            st.dataframe(table_bench, use_container_width=True, hide_index=True)

    # ── Post-Gameweek Error Decomposition (Settled Only) ──────────────────────
    if is_settled:
        st.markdown("### 🔬 Post-Match Residual Analysis")
        ...
        with r1:
            st.markdown("##### 🟢 Top Positive Differentials (Beat Projection)")
        ...
        with r2:
            st.markdown("##### 🔴 Top Variance Leaks (Underperformed Projection)")
