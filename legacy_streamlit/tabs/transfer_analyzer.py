...
def render_transfer_analyzer_tab(conn, events_df, current_gw):
    ...
    available_market_df = league_eval_df[
        ...
        (league_eval_df["avg_mins"] >= min_avg_mins)
    ...
    # ── Simplified 2-Box Transfer Controls ────────────────────────────────────
    st.markdown("#### 🎯 Transfer Rules")
    col_pos, col_neg = st.columns(2)

    with col_pos:
        selected_positive = st.multiselect(
            "🔒 Keep in Squad / 🎯 Target Signings",
            options=pos_options,
    ...
    with col_neg:
        selected_negative = st.multiselect(
            "🔴 Force Sell / ⛔ Blacklist Market",
            options=neg_options,
            format_func=lambda k: neg_labels.get(k, k),
            default=[],
            help="Select squad players you want to sell first and market players you want to block.",
        )
    ...
    total_cost_diff = sum(s["cost_diff"] for s in swaps)
    ...
    with results_slot.container():
        ...
        m3.metric("Remaining In Bank", f"£{(bank_balance - total_cost_diff):.1f}m")
