    ...
    async def load_squad(self, force_refresh: bool = False):
        async with self:
            if not self.manager_id:
                return
            # Fast return if data is already cached and this is not a force refresh for the same manager
            if self.has_data and not force_refresh and self.manager_id == self.last_loaded_manager_id:
                return
