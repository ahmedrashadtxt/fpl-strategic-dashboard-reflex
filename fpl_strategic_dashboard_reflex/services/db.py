"""SQLite database engine, connection handling, and core FPL domain queries.
Completely decoupled from Streamlit and UI frameworks.
"""
...
def ensure_database_ready():
    """Checks if fpl.db exists and contains tables."""
    db_path = get_db_path()
    needs_init = False
    ...
    if needs_init:
        try:
            from fpl_strategic_dashboard_reflex.services import fetch_data
            if hasattr(fetch_data, "fetch_all_data"):
                fetch_data.fetch_all_data()
            elif hasattr(fetch_data, "main"):
                fetch_data.main()
